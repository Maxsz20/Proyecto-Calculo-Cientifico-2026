...
\begin{itemize}
	\item \textbf{Interpolación mediante spline cúbico natural (área entre funciones).} Para interpolar las curvas $f(x)$ y $g(x)$ a partir de los nodos que el usuario coloca sobre la imagen, se utiliza un spline cúbico con condición de frontera natural. Cuando el usuario define líneas de división, la interpolación se realiza por tramos: en cada subintervalo se construye un spline cúbico independiente con los nodos que le corresponden. Un spline cúbico es, esencialmente, una función polinómica a trozos de grado~3 que pasa por todos los nodos y cuya primera y segunda derivada son continuas en los puntos de unión; la condición ``natural'' significa que $S''(x_0) = S''(x_n) = 0$ en los extremos, de modo que la curva no se fuerza a curvarse en los bordes \cite{burden02, cheney08}. En la práctica se emplea la clase \texttt{CubicSpline} del módulo \texttt{scipy.interpolate} \cite{scipy_cubicspline} con el parámetro \texttt{bc\_type="natural"}; internamente, esta rutina plantea y resuelve un sistema tridiagonal de ecuaciones para obtener los coeficientes de cada trozo cúbico, lo que resulta eficiente ($O(n)$) gracias a la estructura de banda de la matriz \cite{burden02}.

	\item \textbf{Interpolación local de Lagrange por ventanas (área entre curvas cerradas).} Para las curvas cerradas parametrizadas, se interpola con polinomios de Lagrange de forma local: en cada segmento paramétrico $[t_i, t_{i+1}]$ se toma una ventana de $d+1$ nodos vecinos (con $d=3$) y se construye un polinomio de Lagrange solo con esos nodos. De esta manera se obtienen los valores $x(t)$, $y(t)$ y sus derivadas $x'(t)$, $y'(t)$ de forma analítica mediante (\ref{eq:deriv_lagrange}), sin recurrir a un polinomio global de grado alto que podría oscilar \cite{cheney08}.
